...
public class Front {
	...
	public void login_page() {
		...
		if( result == -1 ) { System.out.println("[알림] 로그인 실패, 패스워드 불일치"); }
		else if( result == -2 ) { System.out.println("[알림] 로그인 실패, 아이디 불일치"); }
		else { System.out.println("[알림] 로그인 성공, 환영합니다."); board_page(); }
	...
	public void board_page() {
		...
		while( true ) { // 무한루프 [ 종료조건 : 3 입력시 break; ]
			...
			for( int j=0; j<Mcontroller.getInstance().getLoginSession().getBoardlist().size(); j++ ) {
				Board temp = Mcontroller.getInstance().getLoginSession().getBoardlist().get(j);
				// System.out.println( temp ); // 순환참조 걸림
				System.out.println( temp.getTitle() ); // 순환참조 안걸림 
			}
			
			// 출력부 [ 전체 출력 ] 
			int i = 0; // 인덱스 용도
			for( Board b : Bcontroller.getInstance().getList() ) { // 컨트롤 누른상태에서 함수클릭 -> 해당 함수로 이동
	...
	public void delete_page( int bno ) {
		// 1. 유효성검사 [ 현재 보고있는 글의 작성자와 현재 로그인된 회원과 같으면 ]
