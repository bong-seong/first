...
public class Ex1_p466 {
	...
	// 1. p.466
		...
		// 접근제한자 키워드 반환타입 함수명( 타입 매개변수1, 타입 매개변수2 .... ) { return 데이터; }
			// 접근제한자 : public , private , default[생략가능] , protected
			// 키워드[생략가능] : static[정적] , final[오버라이딩금지]
			// 반환타입 : return 할 데이터의 타입
			// 함수명 : 카멜표기법
			// 매개변수 : 타입에 맞춰서 넣기
			
	public static void printLength( String data ) {
		try {	// 예외가 발생할것 같은 코드를 try{} 안에 넣기
		...
		finally { // * 예와가 있던말던 실행되는 코드
			System.out.println("마무리 실행");
		}
	}
		// 1. main함수 선언 : main 스레드가 포함된 함수 -> 스레드 : 코드를 읽어주는 흐름단위 [ 단일 스레드 ]
...
/*
	...
	예외 : 에러 이외 예외라는 오류 
		...
		* 잘못된 사용 : 사용자가 비정상적인 행동했을때 -> 경험
			1. 일반 예외 : 컴파일전에 발생할수 있는 예외 검사 [ * 자바 외부 통신 [스트림] ]
			2. 실행 예외 : 컴파일후에는 발생하는 예외
	예외클래스
		Exception 클래스 : 모든 예외를 받을 수 있는 예외중 최상위 클래스
	문법
		try{
			1. 예외가 발생할것 같은 코드 [ 경험 ] 
		}
