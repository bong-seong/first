...
public class System클래스 {
	public static void main(String[] args) throws Exception {
		...
		for( int i=1; i<=1000000; i++ ) { // 백만번 반복
			sum += i;	// i를 sum에 누적 합계 더하기
		}
		long time2 = System.nanoTime(); // 현재시간을 나노초로 표현
		System.out.println( (time2-time1)  + "나노초가 소요 되었습니다.");
		// 실행전 구한 나노초 time1 , 실행후 나온 나노초 time2 
		// time2( 실행후나온 나노초 ) - time( 실행전 나온 나노초) = 결과 위 코드가 실행된 시간값을 구할 수 있다.
		...
		for( Object obj : props.keySet() ) { // 시스템 속성들의 키를 반복문 돌리기
			System.out.println( (String)obj + "\t\t\t\t"+ System.getProperty( (String)obj ) ); 	// 해당 키를 문자열 반환
		}																						// 해당 키를 이용한 시스템 속성 정보 호출 
		...
		// 문자열 -> 바이트열 로 변환 getBytes("EUC-KR")	인코딩타입 : euc-kr [ 한글 2바이트 ]
