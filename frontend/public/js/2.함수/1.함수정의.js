	...
	함수 : 
		...
		2. 역할
			...
			4. 함수를 만들어야 하는 경우의 수
				...
				3. 영구저장 없이 자심 사용 후 메모리 초기화 해야하는 경우  
		...
		4. 함수 호출 방법
			1. 
				함수명( 인수1, 인수2, 인수3 )
			2. 주의사항 : 정의된 인수의 개수와 동일
				function 함수1( x ){}  ------> 함수1( x )
				function 함수2( x, y ){} ----> 함수2( x,y )
				finction 함수3( x, y, z ){} -> 함수3( x,y,z )
...
function 함수2( x , y ){ // 인수[함수 안으로 들어오는 수/코드] 인수가 2개인 함수 정의
	console.log( "함수2 에서 실행된 결과 : " + x + y )
} // 함수 end
...
// 3. 함수의 반환 [ 3. ]
function 함수3( x , y , z ){ // 인수가 3개인 함수 정의
	let 결과 = x + y + z ; // 함수 정의
	return 결과 ;	
} // 함수
let 결과 = 함수3( 1 , 2 , 3 )
