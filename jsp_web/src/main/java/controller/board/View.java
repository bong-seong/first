...
public class View extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 	- 하루에 한번씩 조회수 또는 좋아요수 또는 싫어요수 증가 기능
		 	- 회원별 vs pc별
		 	- 세션 사용
		 */
		// 1. pc 네트워크 식별자 [ ip ]
		...
		if( o == null ) { // 세션이[bno+type+ip] 생성된 적이 없다.
		
			// 2. 세션 생성 [ ip + type + bno ]
			request.getSession().setAttribute( type + ip + bno , 1 );
				// 3. 세션 생명주기 [ 허용시간 지나면 자동으로 메모리 삭제 / 초 단위 ]
					// 1. 세션 초기화 되는 기준 
						// 1. 서버 끄면 / 2.서버 다시켰을때 / 3. MaxInactiveInterval 설정한 시간이 지났을때
