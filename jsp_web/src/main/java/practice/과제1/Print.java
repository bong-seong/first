...
public class Print extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( type.equals("0") ) { // 타입이 0 일때 전체 사원 출력
			...
			ArrayList<Mdto> result = printDao.getInstance().search(search);
				System.out.println("like검색 : " + result.size() );
			ObjectMapper mapper = new ObjectMapper();
			String jsonArray = mapper.writeValueAsString(result);
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String path = request.getSession().getServletContext().getRealPath("/practice/과제1/img");
		// 2. 객체
