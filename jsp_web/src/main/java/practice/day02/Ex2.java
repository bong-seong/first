 ...
 */
//urlPatterns = "/StudentServlet" , loadOnStartup = 2
@WebServlet( urlPatterns = "/Ex2" , loadOnStartup = 1)
public class Ex2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Dto> list = Dao.getInstance().getData2();
			...
			// * JAVA객체와 JS객체는 체계/형태 가 다르다 [ 호환 불가 ]
			// 해결 : JAVA객치 JS객체로 바꾸자 [ Object -> json ]
		...
		System.out.println( list );
			// [practice.day02.Dto@7c970a1f, practice.day02.Dto@7ee0cd2a, practice.day02.Dto@6e7fa90d, practice.day02.Dto@71914a48]
		System.out.println( jsonArray ); // 자바 객체가 json 형식으로 문자열 변환
			// [ { } , { } ]
		// 2. 요청받은 결과를 JS에게 전달
		response.setCharacterEncoding("UTF-8");
		response.setContentType("application/json"); // 전달[전송]타입 json 명시
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. request.getParameter("매개변수명");	: 데이터를 가져올때 무조건 String 타입으로 가져온다. [ 필요에 따라 형변환 ]
		...
		// 3. DAO 메소드에 인수 10개 전달후 true / false 결과 받기
