	...
	$.ajax({
		...
		success : (r)=>{
			...
			r.forEach( (o,i)=>{
				html +=	`<tr>
							<th width="10%"> ${o.no} </th>
							<th width="10%"> 이미지주소값 </th>
							<th width="10%"> ${o.name} </th>
							<th width="10%"> ${o.type} </th>
							<th width="10%"> ${o.indate} </th>
							<th width="10%"> ${o.outdate} </th>
							<th width="10%"> ${o.outreason} </th>
...
function onclickUpdate(no){
	...
	let html = `
		<form class="updateForm">
			<table class="updateTable" border="1px">
				<tr>
					<th width="10%"> 번호 </th>
					<th width="10%"> 사진 </th>
					<th width="10%"> 이름 </th>
				...
				<tr>
					...
					<th width="10%"> <input type="text" class="name" name="name"> </th>
					<th width="10%"> 
...
function onLeave(no){
	...
	$.ajax({
			url : "/jspweb/print",
			method : "post",
			data : {"no" : no , "leavereason" : leavereason},
			success : (r)=>{
